  ...
  return (
    <div className="bugs">
      ...
      <div onClick={() => handleClick()} className="dropDownInput">
        <p>select Produt(s)</p>
        {checkedItems.length === 1 && `${checkedItems.length} product Selected`}
        {checkedItems.length > 1 && `${checkedItems.length} products Selected`}
      </div>
      {showDropDown && (
        <div className="dropDownContent">
          <label onChange={(e) => handleCheck(e)} htmlFor="product1">
            <input value="product1" id="product1" type="checkbox" />
            product1
          </label>
          <label onChange={(e) => handleCheck(e)} htmlFor="product2">
            <input value="product2" id="product2" type="checkbox" />
            product2
          </label>
          <label onChange={(e) => handleCheck(e)} htmlFor="product3">
            <input value="product3" id="product3" type="checkbox" />
            product3
          </label>
        </div>
      )}
